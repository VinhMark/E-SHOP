...
const CreateProduct = () => {
  ...
  const handleSubmit = (e) => {
    ...
    for (const [key, value] of Object.entries(product)) {
      if (key === 'images') {
        product.images.forEach((i) => formData.append(key, i));
        continue;
      }
  ...
  const handleRemoveImg = (index) => {
    ...
    const arrayImg = Array.from(images.files);

    const dataTransfer = new DataTransfer();
    URL.revokeObjectURL(arrayImg.splice(index, 1));
    ...
    setProduct((prev) => ({ ...prev, images: Array.from(images.files) }));
  };

  return (
    <div className='bg-white w-full flex justify-center'>
      <div className='w-[90%] 800px:w-[50%] p-3'>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className='mt-2'>
            ...
            <input type='file' name='images' id='images' multiple className='hidden' onChange={handleChangeInput} />
            <div className='flex flex-wrap'>
              {product.images &&
                product.images.map((item, i) => (
                  <div
                    className='m-2 relative border-[#555] border-[3px] rounded-md outline-none h-[120px] w-[120px]'
                    key={i}
                  >
          ...
          <div className='mt-2'>
            <button
              ...
              } appearance-none text-center flex items-center justify-center w-full py-5 h-[35px] border border-gray-300 rounded-[3px]focus:outline-none focus:ring-blue-500 bg-gray-200`}
            >
              {loading && (
                <div role='status'>
                  <svg
